var Vivo = function( posicion, source, equipo ){
	...
	this.golpear = function(){
		if( !this.realizando_accion ){
			...
			var perdida_velocidad = this.fuerza-this.equipo.arma.peso;
			if( perdida_velocidad < 0 ){
				/*Se usa para que no se sumen valores*/
				perdida_velocidad = this.velocidad;
			};
			this.velocidad -= perdida_velocidad;//Dismunuimos golpe
	...
	this.loop_obj = function(d){
		...
		if( !this.stunt && this.velocidad > 0 ){
