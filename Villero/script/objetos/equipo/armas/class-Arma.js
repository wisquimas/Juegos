		...
		for( var i = 0; i < objetos.length; i++ ){
			...
			if ( ok ){
				if( tipo_objeto[0] == objetos[i].tipo ){
					...
					if( objetos[i].faccion == 2 || golpeador.faccion != objetos[i].faccion ){
						/*SI ES ENEMIGO PÚBLICO O ES DE UNA FACCIÓN DISTINTA*/
						objetos[i].perder_vida(fuerza_golpe);
					};
				};
				if( tipo_objeto[3] == objetos[i].tipo ){
					/*GOLPE A UN NPC VENDEDOR*/
					objetos[i].interactuar(golpeador.direccion, objetos[i].mensajes.golpe_recibido.misma, golpeador.faccion);
				};
			};
		};
	};
